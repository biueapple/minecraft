...
public class Topography : MonoBehaviour
{
    ...
    public IEnumerator CreateDownTo()
    {
        ...
        for (int i = 0; i < Width_X; i++)
        {
            ...
            {
                topY = -1;
                for (int k = Height_Y - 1; k >= 15; k--)        //최소높이를 정하는곳
                {
                    if (topY != -1)
                    {
                        if (topY - k <= 2)
                        {
                            ...
                            //blocks[i, k, j] = Instantiate(Tparent.BlockPre[0], new Vector3(i + plusX, k, j + plusZ), Quaternion.identity, transform);
                        ...
                        else
                        {
                            ...
                            //blocks[i, k, j] = Instantiate(Tparent.BlockPre[1], new Vector3(i + plusX, k, j + plusZ), Quaternion.identity, transform);
